...
# devName = ["dev_16", "dev_18", "dev_17"]
devPlace = ["kostka", "biodro", "nadgarstek"]
gyroCut = [200, 500, 0, 1000]
    
# title = "Gyro - " + typ + " " + howManyLegs[0]
...
for how_i, (how) in enumerate(howManyLegs):
    title = "Gyro - " + typ + " " + how
    fig = make_subplots(rows=3, cols=3, column_titles=devPlace ,row_titles=peopleName)
    # fig = make_subplots(rows=3, cols=3, column_titles=devName, row_titles=gyro)
    fig.update_layout(title_text=title, showlegend=False)
        ...
        if(os.path.isdir(filePath)):
            ...
            for dev_i, (dev) in enumerate(devices):
                ...
                for qw_i, (qw) in enumerate(gyro):
                    ...
                    y = y[gyroCut[how_i]:gyroCut[how_i]+N]
                    ...
                    fig.add_trace(go.Scatter(x=x, y=y, name=p, mode='lines', line=devLine), row=qw_i+1, col=dev_i+1)
